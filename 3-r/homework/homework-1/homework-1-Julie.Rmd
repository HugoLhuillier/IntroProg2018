...
data = data.frame(read.csv(paste0(thepath,filename),sep=",", header=F))
```
...
last_three_patients = data[1:3,]
last_three_patients 

#first_seven_days = summary(data[,1:7])
#knitr::kable(first_seven_days, digits=2)

library(sjmisc)
lab = c("variable","min","max","mean","sd")
descr(data[,1:7], max.length=1)[,lab]
...
1. Create a new data frame in which you have the first five days’ values in __only__ those patients (remember how to subset a data frame? that's no more complicated than that!)
1. Print out the corrected data frame to check that your code has fixed the problem

```{r, echo = TRUE}
p = as.numeric(dim(data)[1])

#1. Write a vector containing affected patients
patients = seq(from=2, to=p, by = 2)

#2 + 3. Create a new dataframe and print it
correc = subset.data.frame(data, attributes(data)$row.names %in% patients, select = c(V1, V2, V3, V4, V5))
correc
...
apply(data[,1:10], 2, median)

#2. Compute the max for every even days

apply(data[,c(seq(1,40,2))],2, max)

#3. Compute the mean for patients 1 to 5 (over the whole 40 days)

#apply(data[1:5,], 1, mean)
rowMeans(data[1:5,])
...
plot(
  apply(data,2, min), 
  type = "s", 
  main = "Evolution of the minimum inflammation observed", 
  xlab = "Time(days)", 
  ylab = "Minimal inflammation", 
  col="brown", 
  lwd = 2)
```
...
boxplot(
  data,
  main = "BoxPlot representing inflammation over the period",
  xlab = "Time (days)",
  ylab = "Inflammation"
)
