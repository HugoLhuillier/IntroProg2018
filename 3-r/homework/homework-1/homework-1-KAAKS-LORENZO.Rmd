...
```{r, echo = TRUE}
setwd("/Users/lorenzo/Documents/Master in Economics/Programming Course/Session 4/data")
d=read.csv(file="inflammation-02.csv", header=FALSE)
```
...
```{r, echo = TRUE}
apply(d[, 1:10], 2, median)
evendays=seq(from = 2, to = 40, by = 2)
apply(d[, evendays], 2, max)
apply(d[1:5, ], 1, mean)
```
...
```{r, echo = TRUE}
plot(apply(d, 2, min), xlab = "day", ylab = "minimum", main = "minimum inflammation per day")
```

For fun, you can also try the `boxplot` command. (once again, you may want to check `help(boxplot)` if the first place).

```{r, echo = TRUE}
boxplot(apply(d, 2, min), xlab = "day", ylab = "minimum", main = "minimum inflammation per day") 
```
