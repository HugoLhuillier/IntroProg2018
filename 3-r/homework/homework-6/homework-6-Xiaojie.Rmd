...
whonew <- who %>%
          gather(starts_with("new"),key="new",value="count") %>%
          filter(!is.na(count))
...
whonew$new=str_replace(whonew$new,"[w]","w_")
...
whonew <- select(whonew,country,iso2,type,sex,age,count,year)
...
whonew$type=as.factor(whonew$type)
whonew$sex=as.factor(whonew$sex)
whonew$age=as.factor(whonew$age)
whonew=arrange(whonew, age)
```
